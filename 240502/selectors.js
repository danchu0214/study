...
$(document).ready(function ( ) {
    ...
    $('body *').css({'border':'2px solid #000', 'padding':'10px', 'margin': 10});
    ...
    $('.p2').prev().css({ backgroundColor:'tomato'});
    
    $('.p2').next();//.p2의 뒤에 작성된 형제 노드 탐색
    $('.p2').next().css({ backgroundColor: 'salmon'});
    ...
    $('.part').filter('ul');// .part중 ul을 거르겠다
    $('.part').filter('ul').css({ borderColor: '#ECCA9C' })
